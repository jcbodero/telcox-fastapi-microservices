...
export default function Home() {
  ...
  const loadData = async () => {
    ...
    setErrorMessage('')
    try {
      const [customersData, servicesData, catalogData, invoicesData, paymentsData, notificationsData, auditData, ordersData] = await loadAllData()
  ...
  const login = () => {
    setUser({ id: 'mock-user', full_name: 'Usuario TelcoX', email: 'demo@telcox.com' })
    appendLog('Demo user signed in')
  }

  const logout = () => {
    setUser(null)
    appendLog('Usuario desconectado')
  }

  const createCustomer = async () => {
    try {
      const payload = { full_name: 'Mario Perez', document_id: '0912345678', email: 'mario.perez@telcox.com', phone: '+593987654321' }
      const data = await postJson('customer', apiMap.customer, payload)
  ...
  const performOnboarding = async () => {
    ...
    try {
      ...
      const data = await postJson('onboarding', apiMap.onboarding, payload)
  ...
  const processPayment = async (invoice) => {
    ...
    try {
      ...
      const data = await postJson('payment', apiMap.payment, payload)
  ...
  const generateInvoice = async () => {
    ...
    try {
      ...
      const data = await postJson('billing', apiMap.billingGenerate, payload)
  ...
  const createProvisioningOrder = async (product) => {
    ...
    try {
      ...
      const data = await postJson('provisioning', apiMap.provisioning, payload)
  ...
  const sendNotification = async (customerId, channel, message) => {
    try {
      ...
      await postJson('notification', apiMap.notification, payload)
    ...
    return (
      <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(14,158,239,0.18),transparent_28%),linear-gradient(to_bottom,#f8fbff_0%,#eef2ff_100%)] flex items-center justify-center p-6 text-slate-900">
        <div className="w-full max-w-md rounded-[32px] border border-slate-200/80 bg-white/90 p-8 shadow-telecard">
          ...
          <p className="text-slate-700 mb-6">Inicia sesión para ver el estado de tus servicios y administrar tus recursos.</p>
